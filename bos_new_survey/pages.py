...
class Survey(Page):
    ...
    def before_next_page(self):
        ...
        for n, pref in zip(indices, form_fields):
            ...
            self.participant.vars['player_intents'][n - 1] = int(choice_i)

 # METHOD: =================================================================================== #
    # CONTROL PREFS: INTENTIONS MUST BE UNIQUE ================================================= #
    # =========================================================================================== #
    def error_message(self, values):
        indices = [list(i) for i in zip(*self.participant.vars['form_fields_plus_index'])][0]
        form_fields = [list(i) for i in zip(*self.participant.vars['form_fields_plus_index'])][1]
        sum_of_intents = sum([values[i] for i in form_fields])

        if sum_of_intents != sum(indices):
...
class Decision(Page):
    ...
    def error_message(self, values):
        ...
        if sum_of_prefs != sum(indices):
            return '希望はそれぞれ異なる 1 〜 %s の間の英数字で入力してください。' % (indices[-1])
